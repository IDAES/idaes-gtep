...
class ExpansionPlanningSolution:
    ...
    def set_config_defaults(self):
        ...
        # set default values
        self.config["model_specific_nomenclature"] = {
            "bus_descriptor": "bus",
        }
        self.config["period_categories"] = {
    ...
    def _level_relationship_df_to_plot(
        ...
        if all_binaries:
            ...
            for override_category in self.config["binary_order_overrides"]:
                # check that everything can be mapped over
                matched_config_override = True
                for item in vars:
                    if not item in eval(
                        self.config["binary_order_overrides"][override_category]
                    ):
    ...
    def _level_plot_workhorse(
        ...
        for vars_of_interest, keys_of_interest in level_relationships.items():
            ...
            # check the KOIs against the config for any overides
            for override_category in self.config["diagnostic_keys_of_interest"]:
                # check that everything can be mapped over
                override_set = set(
                    eval(self.config["diagnostic_keys_of_interest"][override_category])
                )
                check_set = set()
                # if there are SOME keys but not ALL keys, override the tmp_koi with the one in config
                for item in tmp_koi:
                    # if item in eval(
                    #     self.config["diagnostic_keys_of_interest"][override_category]
                    # ):
                    if item in override_set:
                        check_set.add(item)
                # check if all the matched_keys are in the override_category
                if override_set == check_set:
                    tmp_koi = sorted(
                        eval(
                            self.config["diagnostic_keys_of_interest"][
                                override_category
                            ]
                        )
                    )
    ...
    def _plot_graph_workhorse(
        ...
        tmp_buses_of_interest = self.data.data["elements"]["bus"]
        # check the overrides to see if we need to grab specific buses

        for this_koi in key_of_interest:
            from_bus = self.data.data["elements"]["branch"][this_koi]["from_bus"]
            to_bus = self.data.data["elements"]["branch"][this_koi]["to_bus"]
        ...
        # for item in tmp_buses_of_interest:
        #     # cycle through the branches of interest to find the buses of interest
        #     for this_branch_key in self.data.data["elements"]["branch"]:
        #         if item in [
        #             self.data.data["elements"]["branch"][this_branch_key]["from_bus"],
        #             self.data.data["elements"]["branch"][this_branch_key]["to_bus"],
        #         ]:
        #             G.add_node(item)
        #             labels[item] = item
        #             break
        ...
        def draw_single_edge_flow(
            ...
            def generate_flow_glyphs(
                ...
                glyph_anchor_coord = [(0.5) / float(num_glyphs), 0]
                glyph_verts = 5
                ...
                yscale_transform = Affine2D().scale(sx=0.25, sy=0.25 / glyph_radius)
                # rescale y to make it fit in a 1x1 box
                flow_glyphs[-1].set_transform(yscale_transform)

                # # [XXX] make the start marker a thunderbird because thom has dumb ideas
                # glyph_anchor_coord = [(0.5) / float(num_glyphs), 0]
                ...
                # now cycle through the periods and make the full line
            ...
            bot_flow_glyphs = generate_flow_glyphs(
                weights_bot, glyph_type=glyph_type, glyph_rotation=(np.pi)
            )  # for custom
        ...
        for item in key_of_interest:
            ...
            try:
                ...
                try:
                    ...
                    print(
                        f"Attempted to slice DF in network twice using {edge_key} and {alt_edge_key}, failed both."
                    )
    ...
    def plot_levels(self, save_dir="."):
        ...
        # [XXX] TODO
        # # special handling for buses and extensions
        # # if we've specified that we have both a specific bus set of interest and want to do some number of extensions
        # if self.config.has_option(
        #     "power_flow", "bus_extension"
        # ) and self.config.has_option(
        #     "diagnostic_keys_of_interest", "buses_of_interest"
        # ):
        #     # for this_
        #     # # go through each branch and find any that are "to" or "from" the bus we care about
        #     # for this_branch in self.data.data["elements"]["branch"]
        #     #     from_bus = self.data.data["elements"]["branch"][this_branch]["from_bus"]
        #     #     to_bus = self.data.data["elements"]["branch"][this_branch]["to_bus"]
        ...
        for this_root_level_key in self.primals_tree:
            if "investmentStage" in this_root_level_key:
                ...
                if eval(self.config["period_categories"]["plot_investment"]):
                ...
                parent_key_string = f"{this_root_level_key}"
                if eval(self.config["period_categories"]["plot_representative"]):
                    self._level_plot_workhorse(
                ...
                for this_inv_level_key in self.primals_tree[this_root_level_key].keys():
                    if "representativePeriod" in this_inv_level_key:
                        ...
                        if eval(self.config["period_categories"]["plot_commitment"]):
                            self._level_plot_workhorse(
                                "commitmentPeriod",
                        ...
                        for this_rep_level_key in self.primals_tree[
                            ...
                            if "commitmentPeriod" in this_rep_level_key:
                                ...
                                if eval(
                                    self.config["period_categories"]["plot_dispatch"]
                                ):
                                    self._level_plot_workhorse(
                                        "dispatchPeriod",
